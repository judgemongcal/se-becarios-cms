...
function ExportRecordsBtn({ type }) {
  ...
  async function handleExportAudit() {
    try {
      ...
      if (response.ok) {
        ...
        window.URL.revokeObjectURL(url);
        a.remove();
        setIsSettingsDownloadSuccess(true);
      } else {
        // Handle error response
        console.error(
          'Failed to export records:',
  ...
  return (
    <button
      className={`${
        type === 'audit' && 'mr-3 md:mr-5 xl:mr-0'
      } bg-brand-black shadow-shadow-db rounded-10 hover:bg-brand-green-dark mb-[1rem] p-4 text-[1rem] font-semibold text-[#FFFFFF] duration-300 md:px-5 md:text-[1.25rem]`}
      onClick={() => typeRoute()}
    >
      Export Records
    </button>
